...
class LCD():
	def __init__(self, numbering_mode=GPIO.BCM, bit_mode=MODE_8BIT, line_num=LINE_NUM_2, char_height=CHAR_HEIGHT_10, pin_rs=None, pin_rw=None, pin_e=None, pins_data=None):
		...
		# Initialization (its all low from the start)
		microSleep(10)
	...
	###	Hard Reset  ###
	#----------------------
		#Force HD44780 to 8-bit mode
		self._write4bits(0b0011)
		self._write4bits(0b0011)
		
		#Set to 4-bit mode if necessary
		if self.bit_mode is MODE_4BIT:
			self._write4bits(0b0010) #still in 8-bit mode atm so only send single 4-bit instruction

		#Clear Screen
		self.clear_disp()
		#if self.bit_mode is MODE_8BIT:
		#	self._write8bits(0b00000001)
		#else:
		#	self._write4bits(0b0000)
		#	self._write4bits(0b0001)

		#Ensure display is on
		self.disp_crsr(0b1,0b1,0b1)

		#self._functionset() #Finally we can run the actual function set
	...
	def _pulse_e(self):
		...
		microSleep(0.25)	# E pulse time is 230nS min
	    	GPIO.output(self.pin_e, 0)
	...
	def _send(self, value, rs_mode):
		...
		microSleep(0.04) # Address Setup Time is 40nS minimum
	...
	def disp_crsr(self, disp=0x01, underline=0x01, blink=0x01):
		...
		NOTE: underln is really  more of a highlight
		"""
	...
	def move_crsr(self, num, direction=RIGHT):
		...
		"""
		#Handle negative num
		if (num < 0) and (direction is RIGHT):
			direction = LEFT
			num = num * -1
		if (num < 0) and (direction is LEFT):
			direction = RIGHT
			num = num * -1
	...
	def move_disp(self, num, direction=RIGHT):
		"""
		-moves <num> spaces  (can be negative)
		-direction: 1 = move crsr right, 0 = move crsr left
